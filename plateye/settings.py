...
USER_AGENT_STRINGS_FILES_DIRS_ROOT_DIR = os.path.join(PKG_ROOT_DIR, 'usragent', 'strings-files')
...
PLATINUM_EYE_BANNER = 'PlatinumEye v3.0 by Skochynskyi Dmytro <skodmy@gmail.com>'
...
USER_AGENT_PARTS = {
        ...
        'mac': {
            ...
            'ext': [
                'Intel Mac OS X %d_%d_%d' % (
                    random.randint(10, 11), random.randint(0, 9), random.randint(0, 5)
                ) for _ in range(1, 10)
            ]
        },
    },
    'platform': {
        ...
        'gecko': {
            'name': [
                'Gecko/%d%02d%02d Firefox/%d.0' % (
                    random.randint(2001, 2010),
                    random.randint(1, 31),
                    random.randint(1, 12),
